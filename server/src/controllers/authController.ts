...
const signToken = (id: Types.ObjectId) => {
    const jwtSecret = process.env.JWT_SECRET || 'some-secret-that will not used';
...
const createSendToken = (user: UserDocument, statusCode: number, res: Response) => {
    ...
    const numberDays = parseInt(process.env.JWT_COOKIE_EXPIRES_IN ?? '50');
    const cookieOptions = {
        expires: new Date(Date.now() + numberDays * 24 * 60 * 60 * 1000),
        httpOnly: true
    };
...
export const protect = catchError(async (req: Request, res: Response, next: NextFunction) => {
    let token;
